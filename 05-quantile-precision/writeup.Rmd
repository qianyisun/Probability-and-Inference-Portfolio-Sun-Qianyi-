...
```{r}
mid.len1[41] # 45th quantile
mid.len2[1] #5th quantile
mid.len3[36] # 40 th quantile
mid.len4[1] # 5th quantile
```
For Distribution 1, when its quantile is the range between 40 and 60, the length of median is nearly close to 0.35.
During the range,the median has the tightest sampling distribution among all the quantiles in Distribution 1.

For Distribution 2, when its quantile is approaching to 5th, the length of median is 0.064277793.
At this time,the median has the tightest sampling distribution among all the quantiles in Distribution 2.

For Distribution 3, when its quantile is approaching to 40th, the length of median is 0.683326.
At this time,the median has the tightest sampling distribution among all the quantiles in the mixture distribution 3.

For Distribution 4, when its quantile is approaching to 5th, the length of median is 0.02783157.
At this time,the median has the tightest sampling distribution among all the quantiles in the mixture distribution 4.

Thus, for distribution 4, when its quantile is 5th, it has least value compared to other three distributions. Thus, the conclusion can be easily obtained that the median  of the distribution 4 has the tightest sampling distribution among all the quantiles.
